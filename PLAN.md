...
- [x] Create a new Xcode project (macOS App, Swift + SwiftUI, minimum deployment target macOS 14).
- [x] Configure the app as a menu-bar-only agent (`LSUIElement = YES` in Info.plist).
- [x] Set up a Swift Package Manager workspace for internal modules:
...
- [x] Add a `Makefile` or `justfile` with common commands (`build`, `test`, `run`, `clean`).
- [x] Configure code signing and entitlements (microphone, accessibility, network-client for model download).
- [x] Add `.gitignore` for Xcode / Swift artifacts.
...
- [x] Trigger on: push to `main`, pull requests targeting `main`.
- [x] Runner: `macos-15` (Apple Silicon).
- [x] Steps:
  1. Checkout repo.
...
- [x] Cache SPM dependencies (`~/Library/Developer/Xcode/DerivedData` and `.build`) to speed up builds.
- [x] Fail the workflow on any compiler warning (`SWIFT_TREAT_WARNINGS_AS_ERRORS=YES`).

### 1.2 Lint Workflow (`.github/workflows/lint.yml`)

- [x] Trigger on: push to `main`, pull requests targeting `main`.
- [x] Run **SwiftLint** (install via Homebrew on the runner).
- [x] Add a `.swiftlint.yml` config to the repo with project-appropriate rules.
- [x] Fail the workflow on any lint violation.

### 1.3 Release Workflow (`.github/workflows/release.yml`)

- [x] Trigger on: push of a version tag (`v*.*.*`).
- [x] Build a release archive (`xcodebuild archive`).
- [x] Export the archive as a signed `.app` bundle.
- [x] Notarize the app using `notarytool` (Apple ID credentials stored as GitHub Actions secrets).
- [x] Package the notarized app into a DMG.
- [x] Create a GitHub Release and attach the DMG as an artifact.
...
- [x] Implement `NSStatusItem` with an SF Symbol icon (e.g. `waveform`).
- [x] Add a menu with items: *Settings…*, *History…*, *Quit Transcribo*.
- [x] Toggle icon appearance between idle and recording states (swap symbol or tint color).

### 1.2 Global Keyboard Shortcut

- [x] Register a global event monitor (`NSEvent.addGlobalMonitorForEvents`) for key-down and key-up of the configured shortcut.
- [x] Also register a local monitor so the shortcut works when Transcribo itself is focused.
- [x] Expose a `ShortcutManager` that emits two events: `recordingStarted` and `recordingStopped`.
- [x] Store the chosen shortcut key in `UserDefaults`. Default: Right Option (⌥) key.
...
- [x] Request microphone permission on first use (`AVCaptureDevice.requestAccess`).
- [x] Build `AudioCaptureSession` around `AVAudioEngine`:
...
- [x] Wire `ShortcutManager` events to start/stop the capture session.
...
- [x] On first launch, download model weights to `~/Library/Application Support/Transcribo/models/`.
- [x] Show a progress view (bytes downloaded / total) during download.
- [ ] Verify download integrity with a SHA-256 checksum.
- [x] On subsequent launches, load the cached model. Skip download if checksum matches.
...
- [x] `TranscriptionToken` contains: partial text, `isFinal` flag, confidence score (if available), timestamp.
- [x] Load the model into GPU memory on app launch. Unload on quit.
- [ ] Target latency: partial results within 300 ms of speech.

### 3.4 Language Support

- [x] Accept a `language` parameter (ISO 639-1 code) to configure the model.
- [x] Expose the list of supported languages so the Settings UI can present them.
...
- [x] Prompt the user to grant Accessibility permission (guide them to System Settings → Privacy & Security → Accessibility).
- [x] Detect whether permission is granted at runtime using `AXIsProcessTrusted()`.

### 4.2 Text Insertion

- [x] Primary method: synthesize keyboard events via `CGEvent` to type each character. This is the most universally compatible approach.
...
- [x] Handle streaming: as each `TranscriptionToken` arrives, inject the new characters immediately.
- [x] Handle corrections: if the model revises a partial result, delete the previous partial text (via synthetic backspace events) and re-type the corrected version.
...
- [x] Create a SwiftUI `Settings` scene (opens via menu bar → *Settings…* or ⌘,).
- [ ] **General tab:**
...
- [x] Define a `TranscriptionRecord` model: id, timestamp, transcribed text, source app name, language, duration.
- [x] Store records using **SwiftData** (backed by local SQLite).
- [x] Build a history list view with:
...
- [x] Detect first launch (check `UserDefaults` flag or absence of model files).
- [x] Present a multi-step onboarding flow (SwiftUI sheet or dedicated window):
  ...
  5. ~~**Shortcut Setup** — let user confirm or customize the global shortcut.~~ (skipped — uses default Right Option)
...
- [x] Unit tests for `AudioCaptureSession`, `TranscriptionEngine`, `ShortcutManager`.
...
|Decision|Options|Resolve by|Status|
|--------|-------|----------|------|
|Inference runtime|MLX-Swift vs llama.cpp|End of M4 prototype|Open|
|Persistence layer|SwiftData vs raw SQLite|Start of M6|**Resolved: SwiftData**|
|Default shortcut key|Right Option, Fn, Globe, or other|User testing during M7|**Resolved: Right Option (⌥)**|
|Mac App Store feasibility|Sandbox-compatible or direct-only|During M8|Open|
